package learning.SeleniumFramework.stepDefinitions.ui;

import org.openqa.selenium.WebDriver;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import learning.SeleniumFramework.pageObjects.CheckoutPage;
import learning.SeleniumFramework.pageObjects.PageObjectManager;
import learning.SeleniumFramework.utils.TestContextSetup;

public class CheckoutUISteps {

	public WebDriver driver;
	public CheckoutPage checkoutPage;
	TestContextSetup testContextSetup;
	PageObjectManager pageObjectManager;
	
	
	public CheckoutUISteps(TestContextSetup testContextSetup) {
		this.testContextSetup = testContextSetup;
		checkoutPage = testContextSetup.pageObjectManager.getCheckoutPage();
	}

	
	@When("I select country {string}")
	public void i_select_country(String cntryName) throws InterruptedException{
		checkoutPage.countrySelection(cntryName);
		
	}
	
	@Then("I place the order")
	public void i_place_the_order() {
		
		checkoutPage.placeOrder();
	}

	
}
